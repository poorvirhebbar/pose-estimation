...
def qmult(q1, q2):
    q = torch.zeros(4).float()

    q[0] = q1[0] * q2[0] - torch.dot(q1[1:], q2[1:])
    q[1:] = torch.cross(q1[1:], q2[1:]) + q1[0] * q2[1:] + q2[0] * q1[1:]

    return q


def wrap(func, *args, unsqueeze=False, to_numpy=True):
    """
...
# def rotate_emb(emb, cam_rot_1, cam_rot_2):
#     chunk_size = emb.shape[0]
#     num_points = emb.shape[1]

#     assert (cam_rot_1.shape[0] == emb.shape[0])

#     emb_flat = emb.contiguous().view(-1, 3)

#     cam_rot_1_chunk = cam_rot_1.repeat(num_points, 1)
#     cam_rot_2_chunk = cam_rot_2.repeat(num_points, 1)

#     Rt = wrap(qinverse, cam_rot_2_chunk, to_numpy=False)

#     temp = wrap(qrot, cam_rot_1_chunk, emb_flat, to_numpy=False)
#     emb_rot = wrap(qrot, Rt, temp, to_numpy=False)

#     emb_rot = emb_rot.view(chunk_size, -1, 3)

#     return emb_rot


def rotate_emb(emb, cam_rot):
    chunk_size = emb.shape[0]
    num_points = emb.shape[1]

    assert (cam_rot.shape[0] == emb.shape[0])

    emb_flat = emb.contiguous().view(-1, 3)

    cam_rot_chunk = cam_rot.view(-1, 1, 4).repeat(1, num_points, 1).view(-1, 4)

    emb_rot = wrap(qrot, cam_rot_chunk, emb_flat, to_numpy=False)
