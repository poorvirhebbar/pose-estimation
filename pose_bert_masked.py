...
def step(model, data_loader, criterion, optimizer, split, scheduler=None, get_embed=False):
    ...
    for i, (imgs, seed_seq) in enumerate(tqdm(data_loader, ascii=True)):
        #import pdb; pdb.set_trace()
        ...
        if get_embed:
            ...
            poses.append(pose_3d.detach().cpu())
    ...
    if get_embed:
        #import pdb; pdb.set_trace()
        return torch.cat(poses, 0), torch.cat(embeddings, 0)
        '''
...
if __name__ == "__main__":
    ...
    config = tf.BertConfig(hidden_size=51, num_attention_heads=3,  #8
                           num_hidden_layers=cfg.n_encoders, hidden_dropout_prob=0.0)
    ...
    if cfg.get_embed:
        ...
        for i in range(100):
          ...
          poses_train.append(p_train)
          embeds_train.append(e_train)
          poses_val.append(p_val)
          embeds_val.append(e_val)
        ...
        np.save('saved_pose_embed/mpi_poses_train_seq.npy', poses_train)
        np.save('saved_pose_embed/mpi_poses_val_seq.npy', poses_val)
        np.save('saved_pose_embed/mpi_embed_train_seq.npy', embeds_train)
        np.save('saved_pose_embed/mpi_embed_val_seq.npy', embeds_val)
    ...
    for epoch in range(cfg.n_epochs):
        ...
        if (epoch + 1) % cfg.val_intervals == 0:
            ...
            logger.write(s, 'val')
        if (epoch + 1) % cfg.save_intervals == 0 or epoch>2500:
