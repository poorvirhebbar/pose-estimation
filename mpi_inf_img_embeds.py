...
class MPI_INF(data.Dataset):
    ...
    """
    def get_chunk(self, poses, init_frame=None):
        '''
            poses: n x n_joints x 2 | 3
        '''
    ...
    def __getitem__(self, index):
        ...
        poses_3d, init_frame = self.get_chunk(poses_3d)
        img_embeds = self.annot[index+1][0][init_frame : init_frame + self.max_length]
        ...
        return img_embeds, poses_3d  #, pad_vec, poses_2d

    def __len__(self):
        return self.n_samples

if __name__ == "__main__":
    mpi  = MPI_INF("", split='train', max_length=500)
    dataloader = torch.utils.data.DataLoader(mpi, batch_size=10, shuffle=True)
    for i, (img_embeds, p_3d) in enumerate(dataloader):
        print(i)
        #import pdb; pdb.set_trace()
        print(p_3d.shape)
