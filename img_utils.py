...
def padImage(im, height, width):
    '''
    This function performs padding on the input image so that the
    output is of the size height x width. Since, we're not confident
    about getting the points correct after padding, we'll pad only
    from right and bottom, thus, leaving the keypoint positions intact.

    params:
        im: an Image/featuremap (numpy array) of dim nChannels x h x w
        height: Final height after padding
        width: Final width after padding

    returns:
        padImg: a padded image/featuremap of dimesions
                nChannels x height x width
    '''
    nChann, h, w = im.shape[0], im.shape[1], im.shape[2]
    padImg = np.zeros((nChann, height, width))
    padImg[:, :h, :w] = im
...
#  print('type isssssss',type(scale))
...
def Crop(img, center, scale, rot=0, res=224, name=None):
  ht, wd = img.shape[0], img.shape[1]
  tmpImg = img.copy()
  newImg = np.zeros((res, res, 3), dtype = np.uint8)
  ...
  return newImg.astype(np.float32)
