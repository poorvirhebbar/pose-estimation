...
def main():
    ...
    mask_train = emb_train == 99999*np.ones((512))
    mask_train = mask_train[:, 0]
    emb_train = emb_train[~mask_train]

    mask_val = emb_val == 99999*np.ones((512))
    mask_val = mask_val[:, 0]
    emb_val = emb_val[~mask_val]

    #poses_train = poses_train.reshape(b, n, n_joints*3)
    poses_train = poses_train.reshape(b*n, 10,  n_joints,3)
    poses_train = poses_train.reshape(b*n, 10, n_joints, 3)
    poses_train = poses_train[~mask_train]
    #poses_train = poses_train - poses_train[:, 14:15]

    poses_val = poses_val.reshape(b_val*n_val, 10, n_joints,3)
    poses_val = poses_val.reshape(b_val*n_val, 10, n_joints, 3)
    ...
    cum = 0
    count =0
    for ind in range(1, 10000, 100):
        q_emb = emb_train[ind:ind+1]
        before = emb_train[:ind]
        after = emb_train[ind+1:]
        dataset = torch.cat((before, after), 0)
        # dataset = emb_train[ind+1:]
        sim = torch.cosine_similarity(q_emb, dataset)
        #import pdb; pdb.set_trace()
        max_ind = sim.argmax()
        #print(max_ind)
        r_pose = poses_train[max_ind]
        q_pose = poses_train[ind]
        mpjpe = (((r_pose - q_pose)**2).sum(-1)**0.5).mean()
        cum += mpjpe
        count += 1
        #print(mpjpe)
    print(cum / count)
